from helpers import *
from collaborative import *
import pickle

def run_USRAVG():
	"""Create subimssion with predicitons of the user's average"""
	
	print("Running USRAVG...")
	
	TRAIN_PATH = "data/data_train.csv"
	TEST_PATH = "data/data_test.csv"

	#load the data
	ratings = format_data(load_data(TRAIN_PATH))
	predictions = format_data(load_data(TEST_PATH))

	#calculate averages
	avgUser, avgMovie, avgGlobal = calculate_averages(ratings)

	#set the predictions to the average of the user ratings
	for userid in predictions:
		for movieid in predictions[userid]:
			score = avgUser[userid]
			predictions[userid][movieid] = score

	#create submisson
	create_submission(predictions,"data/submissions/USRAVG.csv")

if __name__ == "__main__":
	run_USRAVG()